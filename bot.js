...
async function backfill() {
	...
	console.log("Backfill from: " + lastBackfillFromBlock + " to: " + toBlock);
	return await subscribe(lastBackfillFromBlock, toBlock, false);
}
let subs = null, backfillCount = 0, backfillAtempts = 1800, backfillInterval = 5, backfillBlockInterval = 6000;
...
function nftRewardsDataV6Stringify(selectedAddresses=null) {
	...
	for(var i = 0; i < nftRewardsDataV6.length; i++) {
		...
		printString = printString + i + ". " + address + "  GNS pool:  " + poolAmount + "  GNS claim:  " + claimAmount + "  GNS total:  " + Math.round((poolAmount + claimAmount) * 100) / 100 + "<br/>";
	}
	printString = "Total GNS: " + Math.round(totalGns / 1e18 * 100) / 100 + "<br/>" + printString;
	if (selectedAddresses) {
		printString = "Selected address % of total:  " + Math.round(selectedAddressesTotal / (totalGns  / 1e18) * 10000) / 100  + "%<br/>" + printString;
		printString = "Selected address amount:  " + Math.round(selectedAddressesTotal * 100) / 100  + " GNS <br/>" + printString;
		printString = "Selected address:  " + selectedAddresses + "<br/>" + printString;
	}
